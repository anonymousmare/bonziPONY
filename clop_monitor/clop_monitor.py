...
import fourchan
from good_colours import colour_for, colour_for_text
...
class ClopClient:
    ...
    def _fourchan_payload(self) -> object:
        """The configured thread's decoded JSON, or raise saying why not.

        Shared by the alert path, which wants only the newest post, and by
        ``fourchan_thread_posts``, which wants all of them. The archived check lives here rather
        than in either caller: an archived thread is equally fatal to both, and the reader is
        equally in need of being told to update the URL.
        """
        if self.fourchan_thread is None:
            raise MonitorError("No 4chan thread is configured")
        ...
        if fourchan.is_archived(payload):
    ...
    def fourchan_thread_posts(self) -> List["fourchan.ThreadPost"]:
        """Every post in the configured thread, for reading rather than alerting.

        Deliberately does not consume ``initial_fourchan_post``. That baseline exists so the
        first poll does not alert on a post the reader has already seen; reading the thread is a
        separate act that must not quietly disarm it.
        """
        if self.fourchan_thread is None:
            return []
        try:
            return fourchan.parse_thread_payload(self._fourchan_payload())
        except fourchan.ThreadParseError as error:
            raise MonitorError(f"The configured 4chan thread is unreadable: {error}") from error
    ...
    def _latest_fourchan_post(self) -> Optional[FourChanPost]:
        ...
        payload = self._fourchan_payload()
...
#: What ``alert_category`` calls an alert whose ``icon_key`` names a good rather than one of the
#: ``CATEGORY_ICON_KEYS``. Only the market alerts are keyed that way.
ALERT_CATEGORY_MARKET = "market"
#: An alert keyed to nothing at all. ``build_alerts`` does not produce these -- the suite holds it
#: to the documented keys -- but ``notify`` is also handed plain strings by the sheet-sync step.
ALERT_CATEGORY_OTHER = "other"


def alert_category(alert: object) -> str:
    """Which kind of alert this is, as a stable name a caller can group by.

    Derived from ``icon_key`` rather than stored alongside it. ``icon_key`` already decides this
    -- it is either one of ``CATEGORY_ICON_KEYS`` or a good's ``game_name``, and nothing else --
    so a second field would be a second thing to keep right, free to disagree with the first.
    """
    key = getattr(alert, "icon_key", "")
    if key in CATEGORY_ICON_KEYS:
        return key
    if key:
        from goods import BY_GAME_NAME

        if key in BY_GAME_NAME:
            return ALERT_CATEGORY_MARKET
    return ALERT_CATEGORY_OTHER


def alert_parts(alert: object, body_limit: Optional[int] = None) -> Dict[str, Optional[str]]:
    """One alert broken into the pieces any notification surface needs.

    The conventions this reads were the toast path's private business until something else wanted
    to render an alert too: the first line is the heading, because every alert with more than one
    line leads with what it is ("Buy orders for Copper:") and follows with the detail; and a bare
    URL alone on the last line is the thing to open, not body text to show.

    ``body_limit`` clips the body and is the caller's choice -- a toast gets about three lines on
    screen, a panel that scrolls does not, so neither should have to live with the other's limit.
    ``None`` means keep the whole thing.

    ``colour`` is the 2px trim, and is ``None`` whenever no good is in question. A market alert
    names its good in ``icon_key``, so that one is exact; a report is free text the game wrote, so
    it is word-matched (see ``good_colours.find_good_in_text``); messages, news and thread alerts
    are about no good at all and get no trim.
    """
    text = str(alert)
    ...
    if category == ALERT_CATEGORY_MARKET:
        colour = colour_for(getattr(alert, "icon_key", ""))
    elif category == "report":
        # Title and body together, not just the body. A report alert is one line plus a
        # trailing URL, so the whole of what the game said ends up in the title and a
        # body-only match would find nothing to colour.
        colour = colour_for_text(f"{heading}\n{body}")
    else:
        colour = None
...
class Notifier:
    ...
    def _toast_payload(self, alert: Alert) -> Dict[str, str]:
        ...
        The splitting itself is ``alert_parts``, shared with every other surface that renders an
        alert. What stays here is the toast's own two answers: the body is clipped to what a
        toast can show, and the icon is a sprite path, which needs the configured folder.
        Colour is dropped -- a Windows toast cannot be tinted, which is why the sprite exists.
        """
        parts = alert_parts(alert, body_limit=TOAST_BODY_LIMIT)
        return {
            "title": parts["title"] or "CLOP monitor",
            "body": parts["body"] or "",
