...
## Rendering alerts somewhere other than a toast

`alert_parts(alert, body_limit=None)` breaks one alert into the pieces any notification surface
needs — `title`, `body`, `url`, `category`, `colour` — and is what the toast path itself uses. The
conventions it reads were private to that path until something else wanted to draw an alert too:
the first line is the heading, because every alert with more than one line leads with what it is
("Buy orders for Copper:"); and a bare URL alone on the last line is the thing to open, not body
text to show.

`body_limit` is the caller's choice. A toast gets about three lines on screen and passes
`TOAST_BODY_LIMIT`; a panel that scrolls passes nothing and keeps the whole report.

`category` is one of the four documented icon keys (`messages`, `news`, `report`, `4chan`),
`"market"` for the per-good buy-order alerts, or `"other"`. It is derived from `icon_key` rather
than stored beside it — `icon_key` already decides this, and a second field would be free to
disagree with the first.

`colour` is a `#RRGGBB` trim for surfaces that can paint one, and `None` whenever no good is in
question. Windows toasts cannot be tinted, which is why the sprite carries identity there; the
colours live in `good_colours.py` because `resourcedefs` has no colour column and `goods.py`
deliberately knows the game's vocabulary and nothing about display. A market alert names its good
in `icon_key`, so that lookup is exact. A report is free text the game wrote, so it is word-matched
against the goods table, longest name first — which means "Basic Oil Well" reads as Oil. That is
the useful answer often enough, and harmless when it is not, because the alert's text is what the
reader acts on. Messages, news and thread alerts are about no good at all and get no trim.

Fourteen goods have no colour: the twelve DNA variants, Forbidden Research and Apotheosis Serum.
Five colours are shared by two goods each. A trim is a glance-level hint, not an identifier.

## Reading the whole thread

The alert path only ever wants the newest post. `ClopClient.fourchan_thread_posts()` answers the
other question — what has the thread been *saying* — and `fourchan.render_thread_compact()` turns
that into text cheap enough to hand to a language model:

```python
posts = client.fourchan_thread_posts()
print(fourchan.render_thread_compact(posts, since_number=last_seen, board="mlp", thread_id=43381621))
```

Two things keep it small. Posts are renumbered 1..N and quote-links are rewritten to match, which
takes nine digits out of every reply; `ThreadPost.number` keeps the real number so a link can still
be built, and a link to another thread is left alone rather than guessed at. And line breaks are
kept, unlike `parse_fourchan_comment`, which flattens a post to one line — right for a toast body,
wrong here, because greentext is a line-level convention and a post whose `>` lines have run
together reads as gibberish.

`since_number` renders only newer posts, but the renumbering is always computed over the whole
thread, so `>>3` means the same thing whether or not post 3 was rendered this time.

Reading the thread deliberately does **not** consume the startup baseline. That baseline exists so
the first poll does not alert on a post you have already seen, and reading is a separate act that
...
colour, thread, nation, building and stockpile tests (`test_sheets.py`, `test_overview.py`,
`test_goods.py`, `test_good_colours.py`, `test_fourchan.py`, `test_nation.py`, `test_buildings.py`,
`test_stockpiles.py`) stub the network, so they never contact Google or the game. All **592** of
them run under one command:
