...
class AlertPartsTests(unittest.TestCase):
    """How an alert decomposes for anything that has to render it.

    The conventions -- first line is the heading, a bare trailing URL is the link -- used to be
    private to the toast path. They are shared now, so they are pinned here rather than only
    through the toasts that happen to exercise them.
    """

    def test_first_line_is_the_title_and_the_colon_is_dropped(self):
        parts = alert_parts(Alert("Buy orders for Copper:\n  someone wants 5", "Copper"))
        self.assertEqual(parts["title"], "Buy orders for Copper")
        self.assertEqual(parts["body"], "someone wants 5")

    def test_a_single_line_alert_keeps_its_text_as_the_body(self):
        parts = alert_parts(Alert("3 unread user message(s) pending", "messages"))
        self.assertEqual(parts["title"], "CLOP monitor")
        self.assertEqual(parts["body"], "3 unread user message(s) pending")

    def test_a_trailing_url_is_lifted_out_of_the_body(self):
        parts = alert_parts(
            Alert("New CLOP report (now): stuff\nhttps://4clop.org/reports.php", "report")
        )
        self.assertEqual(parts["url"], "https://4clop.org/reports.php")
        self.assertNotIn("http", parts["body"])

    def test_a_url_mid_body_is_not_treated_as_the_link(self):
        # Only a URL alone on the last line is the click target. Anything else is prose.
        parts = alert_parts(Alert("Report:\nsee https://4clop.org/x for why\nmore", "report"))
        self.assertIsNone(parts["url"])

    def test_no_body_limit_by_default(self):
        long_body = "x" * 900
        parts = alert_parts(Alert(f"Report:\n{long_body}", "report"))
        self.assertEqual(len(parts["body"]), 900)

    def test_body_limit_clips_with_an_ellipsis(self):
        parts = alert_parts(Alert("Report:\n" + "x" * 900, "report"), body_limit=50)
        self.assertEqual(len(parts["body"]), 50)
        self.assertTrue(parts["body"].endswith("..."))


class AlertCategoryTests(unittest.TestCase):
    def test_the_documented_category_keys_are_their_own_category(self):
        for key in clop_monitor.CATEGORY_ICON_KEYS:
            with self.subTest(key=key):
                self.assertEqual(alert_category(Alert("x", key)), key)

    def test_a_good_name_means_a_market_alert(self):
        self.assertEqual(
            alert_category(Alert("x", "Machinery Parts")), clop_monitor.ALERT_CATEGORY_MARKET
        )

    def test_an_unkeyed_alert_is_other(self):
        self.assertEqual(alert_category(Alert("x")), clop_monitor.ALERT_CATEGORY_OTHER)

    def test_a_plain_string_is_other(self):
        # sync_sheet_step hands notify() bare strings, not Alerts.
        self.assertEqual(alert_category("Building counts corrected"), clop_monitor.ALERT_CATEGORY_OTHER)


class AlertColourTests(unittest.TestCase):
    """The 2px trim, and when there should not be one."""

    def test_a_market_alert_takes_its_colour_from_the_named_good(self):
        # icon_key is the good's game_name, so this path never has to guess.
        self.assertEqual(alert_parts(Alert("Buy orders for Pies:\n x", "Pies"))["colour"], "#FF00DC")

    def test_a_report_is_word_matched_against_its_body(self):
        parts = alert_parts(Alert("New CLOP report (now): your Tungsten ran out", "report"))
        self.assertEqual(parts["colour"], "#808080")

    def test_a_real_shaped_report_is_matched_from_its_title(self):
        # build_alerts writes a report as one line plus a trailing URL, so everything the
        # game said lands in the title and the body is left empty. Matching the body alone
        # found nothing to colour.
        alert = Alert(
            "New CLOP report (07/24 21:47): You don't have enough Tungsten to run your 6 "
            "Tungsten Mines! (-6 sat)\nhttps://4clop.org/reports.php",
            "report",
        )
        parts = alert_parts(alert)
        self.assertEqual(parts["body"], "")
        self.assertEqual(parts["colour"], "#808080")

    def test_a_report_about_no_good_has_no_trim(self):
        self.assertIsNone(alert_parts(Alert("New CLOP report (now): war declared", "report"))["colour"])

    def test_messages_and_news_never_carry_a_trim(self):
        for key in ("messages", "news", "4chan"):
            with self.subTest(key=key):
                # Even when the text happens to name a good.
                alert = Alert("New 4chan post: anyone selling Copper?", key)
                self.assertIsNone(alert_parts(alert)["colour"])

    def test_a_market_alert_for_an_uncoloured_good_has_no_trim(self):
        self.assertIsNone(alert_parts(Alert("Buy orders for x:\n y", "Forbidden Research"))["colour"])
