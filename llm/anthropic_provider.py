...
class AnthropicProvider(LLMProvider):
    ...
    def call_with_tools(
        ...
        """Run a tool-use loop and return the final text. See LLMProvider.call_with_tools.

        Does not touch self._history: the tool traffic is working-out, not something the
        character should later remember saying. Reuses _call_with_retry unchanged, so the
        same 5xx/429 handling applies to every round.
        """
        ...
        # Text produced alongside a tool call -- the model thinking out loud on its way to
        # the call. Kept only as a fallback: if the loop ends properly there is a real answer
        # to speak, and prefixing it with "let me look that up" is just more TTS.
        interim: List[str] = []
        final = ""
        ...
        for round_index in range(max_rounds):
            ...
            logger.info("[TIMING] tool round %d took %.2fs", round_index + 1, time.time() - t0)
            ...
            for call in calls:
                ...
                try:
                    ...
                    # Handed back rather than raised. A model that asked for a building that
                    # does not exist usually recovers by asking for the right one, and a
                    # traceback here would lose the whole turn.
            ...
            logger.warning("Tool loop hit %d rounds without a final answer", max_rounds)

        return self._strip_think((final or "\n".join(interim)).strip())
